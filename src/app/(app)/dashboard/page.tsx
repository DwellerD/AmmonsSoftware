...
export default function DashboardPage() {
  ...
  // Sprint 3 document attention lists (pinned + recently uploaded).
  ...
  // FUTURE FEATURE:
  // Contractor schedule-confirmation cards (pending/declined), contractor punch
  // updates, and a recent-notifications card were removed from the dashboard for
  // the current MVP — it focuses on GC workflow status, not messaging or a
  // contractor portal. Restore these (and their data fetches) when a contractor
  // self-service portal / messaging is added back.
      ...
      {loading ? (
        ...
        <div className="space-y-6">
          ...
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            ...
            {/*
              FUTURE FEATURE:
              Pending/declined schedule-confirmation cards and a contractor
              punch-update card were removed for the current GC-focused MVP. The
              schedule-confirmation feature still lives on each trade phase page;
              these dashboard cards (and the data they need) can return when a
              contractor self-service portal is added.
            */}
          </div>
          ...
          {/*
            FUTURE FEATURE:
            A "Recent notifications" card was removed from the dashboard for the
            current MVP — the dashboard focuses on project/workflow status, not
            message delivery. Internal notification records are still written for
            activity history. Restore this card when automated messaging or a
            notification inbox is added back.
          */}
        </div>
      )}
    </PageContainer>
